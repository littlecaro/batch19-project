...
function loadChats()
{
    ...
    $str = 'SELECT m.id AS messageId, m.message, m.sender_id, m.datetime, m.conversation_id, c.id AS contactId, c.first_name AS contactFirstName, c.last_name AS contactLastName, c.profile_picture AS contactProfilePicture
FROM messages m
LEFT JOIN users c
...
function getMessages($conversationId)
{
    ...
    $str = 'SELECT u.profile_picture,u.first_name,u.last_name,m.conversation_id, m.id, m.sender_id, m.message, DATE_FORMAT(m.datetime, "%M %d") as datetime FROM messages m INNER JOIN users u ON m.sender_id=u.id WHERE m.conversation_id = :ConversationId ';
    $db = dbConnect();
    $query = $db->prepare($str);
